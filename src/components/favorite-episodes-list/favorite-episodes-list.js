...
class FavoriteEpisodesList extends React.Component {
  ...
  render() {
    ...
    if (favoriteEpisodes.length === 0) {
      return <div className="title-no-favorites d-flex justify-content-center align-items-center px-2 mb-4 ">У вас пока нет избранных эпизодов</div>;
    }

    return (
      <>
        <div className="sort-block d-flex flex-column flex-md-row align-items-center mt-5 px-2 mb-4 ">
          <div className="sort-text px-2" onClick={() => this.filterFavoriteEpisodesHandler('id')}>Сортировать:</div>
          <div
